...
public class TEAUtilWithNoRand {
    private byte plain[];
     ...
     */
    public TEAUtilWithNoRand(byte k[]) {
    ...
    public String encode2HexBase64(byte in[]) {
        return Base64Utils.encodeToUrlSafeString(encode(in));
    }
    ...
    public byte[] decodeFromBase64Str(String code) {
        return decode(Base64Utils.decodeFromUrlSafeString(code));
    }
    ...
    //通过TEA算法加密信息
    public byte[] encryptByTea(byte[] info) {
        int n = 8 - info.length % 8;//若info的位数不足8的倍数,需要填充的位数
        n = n == 8 ? 0 : n;
        byte[] encryptStr = new byte[info.length + n];
        System.arraycopy(info, 0, encryptStr, n, info.length);
        byte[] result = new byte[encryptStr.length];
        for (int offset = 0; offset < result.length; offset += 8) {
            byte[] tempEncrpt = encipher(Arrays.copyOfRange(encryptStr, offset, offset + 8));
    ...
    public byte[] decryptByTea(byte[] secretInfo) {
        int n = 8 - secretInfo.length % 8;//若info的位数不足8的倍数,需要填充的位数
        n = n == 8 ? 0 : n;
        byte[] decryptStr = null;
        byte[] tempDecrypt = new byte[secretInfo.length + n];
        for (int offset = 0; offset < secretInfo.length; offset += 8) {
            decryptStr = decipher(Arrays.copyOfRange(secretInfo, offset, offset + 8));
